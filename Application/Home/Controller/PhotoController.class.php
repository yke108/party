...
class PhotoController extends FController {
	...
	public function photo_listAction(){
        ...
        //查询列表
		$result = $this->PhotoService()->getPagerList($params);
        $newlist = [];
        if($result['list'] != [])
        {
    ...
    public function infoAction(){
        ...
        $result = $this->PhotoService()->findInfo($map);
        if($result)
        {
            //统计阅读数
            $params = [
            ...
            $this->PhotoService()->increaseReadNum($params);
            //收藏状态
            $map = [
                'user_id'=>$post['user_id'],
                'collection_type'=>Status::CollectTypeAlbum,
                'content_id'=>$post['id'],
            ];
            $collect_info = M('collection')->where($map)->find();
        }else{
            throw new CsException('暂无该相册');
        }
        //查询该文章是否已经学习过 如果学习过则返回相应的 状态码 1=未开始 2=开始中 3=已结束
        $studywhere = [
            'id'=>$post['id'],
            'user_id'=>$post['user_id'],
            'type'=>Status::CollectTypeAlbum,
        ];
	...
	public function delAction(){
		...
		try {
			$result = $this->PhotoService()->delete($params);
			$data = array(
					'Message'=>'删除成功',
			);
		} catch (\Exception $e) {
