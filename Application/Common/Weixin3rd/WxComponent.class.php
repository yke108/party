...
class WxComponent
{
    ...
    const WXA_MODIFY_DOMAIN = '/modify_domain?access_token=';
    const WXA_COMMIT = '/commit?access_token=';
    const WXA_GET_QRCODE = '/get_qrcode?access_token=';
    const WXA_GET_TEMPLATE_LIST = '/gettemplatelist?access_token=';
    const WXA_CHANGE_VISIT_STATUS = '/change_visitstatus?access_token=';
    const WXA_GET_TEMPLATE_DRAFT_LIST = '/gettemplatedraftlist?access_token=';
    const WXA_ADD_TO_TEMPLATE = '/addtotemplate?access_token=';
    const WXA_GET_LATEST_AUDITSTATUS = '/get_latest_auditstatus?access_token=';
    const WXA_GET_CATEGORY = '/get_category?access_token=';
    const WXA_SUBMIT_AUDIT = '/submit_audit?access_token=';
    const WXA_SET_WEB_VIEW_DOMAIN = '/setwebviewdomain?access_token=';
    const WXA_JSCODE_SESSION = 'https://api.weixin.qq.com/sns/component/jscode2session?';
    ...
    /**
     * 小程序，获取openid和session_key
     * @param string $appid
     * @param string $code
     * @param string $access_token
     * @return boolean|mixed
     */
    public function getSessionByJscode($appid, $code, $access_token)
    {
    	$url = self::WXA_JSCODE_SESSION .'grant_type=authorization_code&appid='.$appid;
    	$url .= '&js_code='.$code;
    	$url .= '&component_appid='.urlencode($this->component_appid);
    	$url .= '&component_access_token='.$access_token;
     ...
     * 设置小程序服务器域名
     * @param string $access_token
     * @return array {action,requestdomain,wsrequestdomain,uploaddomain,downloaddomain}
     */
    public function modify_domain($access_token, $post_data)
    {
        $result = $this->http_post(self::WXA_URL_PREFIX . self::WXA_MODIFY_DOMAIN . $access_token, json_encode($post_data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return array {action,requestdomain,wsrequestdomain,uploaddomain,downloaddomain}
     */
    public function setwebviewdomain($access_token, $post_data)
    {
        $result = $this->http_post(self::WXA_URL_PREFIX . self::WXA_SET_WEB_VIEW_DOMAIN . $access_token, json_encode($post_data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return array {action,requestdomain,wsrequestdomain,uploaddomain,downloaddomain}
     */
    public function commit_code($access_token, $post_data)
    {
        $result = $this->http_post(self::WXA_URL_PREFIX . self::WXA_COMMIT . $access_token, json_encode($post_data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json) {
                return false;
    ...
    {
        return self::WXA_URL_PREFIX . self::WXA_GET_QRCODE . $access_token;
        $result = $this->http_post(self::WXA_URL_PREFIX . self::WXA_GET_QRCODE . $access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json) {
                return false;
    ...
    {
        $result = $this->http_post(self::WXA_URL_PREFIX . self::WXA_GET_TEMPLATE_LIST . $access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json) {
                return false;
     ...
     */
    public function change_visitstatus($access_token, $action)
    {
        $result = $this->http_post(self::WXA_URL_PREFIX . self::WXA_CHANGE_VISIT_STATUS . $access_token, json_encode(['action'=> $action]));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json) {
                return false;
    ...
    {
        $result = $this->http_post(self::WXA_URL_PREFIX . self::WXA_GET_TEMPLATE_DRAFT_LIST . $access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json) {
                return false;
    ...
    {
        $result = $this->http_post(self::WXA_URL_PREFIX . self::WXA_ADD_TO_TEMPLATE . $access_token, json_encode(['draft_id'=> $draft_id]));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json) {
                return false;
     ...
     * @param string $access_token
     */
    public function get_category($access_token)
    {
        $result = $this->http_post(self::WXA_URL_PREFIX . self::WXA_GET_CATEGORY . $access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json) {
                return false;
     ...
     */
    public function submit_audit($access_token, $post_data)
    {
        $result = $this->http_post(self::WXA_URL_PREFIX . self::WXA_SUBMIT_AUDIT . $access_token, json_encode($post_data, JSON_UNESCAPED_UNICODE));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json) {
                return false;
     ...
     * @param string $access_token
     */
    public function get_latest_auditstatus($access_token)
    {
        $result = $this->http_post(self::WXA_URL_PREFIX . self::WXA_GET_LATEST_AUDITSTATUS . $access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json) {
                return false;
     ...
     * @param string $access_token
     */
    public function get_dome($url, $post_data)
    {
        $param = !empty($post_data) ? json_encode($post_data, JSON_UNESCAPED_UNICODE) : "";
