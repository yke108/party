...
class WxComponentService
{
    ...
    public function getAuthorizeUrl($redirectUrl)
    {
        ...
        return $this->getWxComponent()->get_auth_cb_url($preAuthCode, $redirectUrl);
    }
    ...
    public function authorizeCallbackProcess($authCode, $expireIn)
    {
        ...
        $this->cache->set($authName, $authCode, $expireIn);
        ...
        $this->getWxComponent()->modify_domain($access_token, [
            'action'=> 'set',
            'requestdomain'=> ['https://dj.huiyimob.com'],
            'wsrequestdomain'=> ['wss://dj.huiyimob.com'],
            'uploaddomain'=> ['https://dj.huiyimob.com'],
            'downloaddomain'=> ['https://dj.huiyimob.com'],
        ]);

        // 设置业务域名
        $this->getWxComponent()->setwebviewdomain($access_token, [
            'action'=> 'add',
            'webviewdomain'=> ['https://dj.huiyimob.com'],
        ]);

        $authName = "wxAppAccessToken" . $this->wxComponentAppId . "_" . $authInfo['authorization_info']['authorizer_appid'];
        $this->cache->set($authName, $authInfo['authorization_info']['authorizer_access_token'], $authInfo['authorization_info']['expires_in']);

        $authName = "wxAppRefreshToken" . $this->wxComponentAppId . "_" . $authInfo['authorization_info']['authorizer_appid'];
        $this->cache->set($authName, $authInfo['authorization_info']['authorizer_refresh_token']);
    ...
    public function getAppAccessToken($appId)
    {
        ...
        $authName = "wxAppAccessToken" . $this->wxComponentAppId . "_" . $appId;
        $this->cache->set($authName, $refreshTokenInfo['authorizer_access_token'], $refreshTokenInfo['expires_in']);

        $authName = "wxAppRefreshToken" . $this->wxComponentAppId . "_" . $appId;
        $this->cache->set($authName, $refreshTokenInfo['authorizer_refresh_token']);
    ...
    protected function getPreAuthCode()
    {
        ...
        // $preAuthCode = $this->cache->get($authName);  //使用缓存时，出错TODO
        if ($preAuthCode) return $preAuthCode;
        $componentAccessToken = $this->getComponentAccessTocken();
        $preAuthCodeArr = $this->getWxComponent()->get_preauth_code($componentAccessToken);
        $this->cache->set($authName, $preAuthCodeArr['pre_auth_code'], $preAuthCodeArr['expires_in'] - 10);
    ...
    protected function getComponentAccessTocken()
    {
        ...
        $componentAccessTocken = $this->cache->get($authName);
        if ($componentAccessTocken) return $componentAccessTocken;
        $accessArr = $this->getWxComponent()->get_access_token();
        $this->cache->set($authName, $accessArr['component_access_token'], $accessArr['expires_in'] - 10);
    ...
    {
        $authName = "wxComponentVerifyTicket" . $this->wxComponentAppId;
        $ComponentVerifyTicket = $this->cache->get($authName);
    ...
    public function getOauthAccessTokenForCode($appId)
    {
        ...
        if ($ret) {
            ...
            $this->cache->set($authName, $ret['access_token'], $ret['expires_in']);
            $authName = "wxComponentOauthRefreshToken" . $this->wxComponentAppId . "_" . $appId;
            $this->cache->set($authName, $ret['refresh_token'], 30 * 24 * 2600); // refresh_token30天有效期
        }
        return $ret;
    }
    ...
    {
        $authName = "wxComponentOauthToken" . $this->wxComponentAppId . "_" . $appId;
        $accessToken = $this->cache->get($authName);
        if ($accessToken) return $accessToken;

        $authName = "wxComponentOauthRefreshToken" . $this->wxComponentAppId . "_" . $appId;
        $refreshToken = $this->cache->get($authName);
    ...
    public function getJsTicket($appId)
    {
        ...
        $jsTicket = $this->cache->get($authName);
        if ($jsTicket) return $jsTicket;
        $weObj = $this->getWechat($appId);
        if (!$weObj) return false;
        $json = $weObj->getJsTicket2($appId);
        if ($json) {
            $this->cache->set($authName, $json['ticket'], $json['expires_in']);
            return $json['ticket'];
        }
        return false;
    }
    ...
    public function getJsCardTicket($appId)
    {
        ...
        $jsCardTicket = $this->cache->get($authName);
    ...
    {
    	$componentAccessToken = $this->getComponentAccessTocken();
    	$json = $this->getWxComponent()->getSessionByJscode($appid, $code, $componentAccessToken);
    	if ($json) return $json;
     ...
     * @param string $url
     * @param array $data
     * @param int $type  1：小程序授权TOKEN，2：第三方平台自己TOKEN
     * @return boolean|array
     */
    public function demo($appId, $url, $data, $type = 1) {
        $access_token = $type == 1 ? $this->getAppAccessToken($appId) : $this->getComponentAccessTocken();
        $url = str_replace('TOKEN', $access_token, $url);
        $this->getWxComponent()->get_dome($url, $data);
    }
     ...
     * @return boolean|array
     */
    public function setDomain($appId, $post_data) {
        $access_token = $this->getAppAccessToken($appId);
        $json = $this->getWxComponent()->modify_domain($access_token, $post_data);
        if ($json) return $json;
        return $this->getWxComponent()->errMsg.'>>>:'.$access_token;
    }
     ...
     * @return boolean|array
     */
    public function commitCode($appId, $post_data) {
        $access_token = $this->getAppAccessToken($appId);
        return $this->getWxComponent()->commit_code($access_token, $post_data);
    }
    ...
    public function getQrcode($appId) {
        $access_token = $this->getAppAccessToken($appId);
        return $this->getWxComponent()->get_qrcode($access_token);
    }
    ...
    public function getTemplateList() {
        $componentAccessToken = $this->getComponentAccessTocken();
        return $this->getWxComponent()->gettemplatelist($componentAccessToken);
    }
     ...
     * @return boolean|array
     */
    public function changeVisitStatus($appId, $action) {
        $access_token = $this->getAppAccessToken($appId);
        return $this->getWxComponent()->change_visitstatus($access_token, $action);
    }
    ...
    public function getTemplateDraftList() {
        $componentAccessToken = $this->getComponentAccessTocken();
        return $this->getWxComponent()->gettemplatedraftlist($componentAccessToken);
    }
    ...
    public function addToTemplate($draft_id) {
        $componentAccessToken = $this->getComponentAccessTocken();
        return $this->getWxComponent()->addtotemplate($componentAccessToken, $draft_id);
    }
    ...
    public function getCategory($appId) {
        $access_token = $this->getAppAccessToken($appId);
        return $this->getWxComponent()->get_category($access_token);
    }
    ...
    public function submitAudit($appId, $data) {
        $access_token = $this->getAppAccessToken($appId);
        return $this->getWxComponent()->submit_audit($access_token, $data);
    }
    ...
    public function getLatestAuditstatus($appId) {
        $access_token = $this->getAppAccessToken($appId);
        return $this->getWxComponent()->get_latest_auditstatus($access_token);
    }
}
