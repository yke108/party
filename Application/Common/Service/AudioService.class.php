...
class AudioService{
    ...
    public function getPagerList($params){
        ...
        return array(
            'list'=>is_option($list,Status::CollectTypeAudio),
            'count'=>$count,
            'page'=>$params['page'],
    ...
    private function setDetail($list)
    {
        if(empty($list)) {
            ...
            $map['cat_type'] = Status::CatTypeAudio;
            //搜索分类表 取分类名字
	...
	public function getWhPagerList($params){
        ...
        $map['region_code']  = $params['region_code'];
        $where['ref_id'] = ['neq',0];
...
//
    ...
    public function createOrModify($params){
        ...
        $data = array(
            ...
            'audio_url'     =>trim($params['audio_url']),
    ...
    public function findInfo($map){
        $info = $this->listDao()->findRecord($map);
        if (empty($info)) {
            throw new CsException('音频不存在', CodeConst::DataNotExist);
        }
    ...
    private function setInfo($info)
    {
        if(empty($info)) {
            ...
            $newinfo['AudioUrl'] = $info['audio_url'];
    ...
    public function increaseReadNum($params){
        ...
        if ($params['user_id']) {
            $map = array(
                ...
                'article_type'=>Status::CatTypeAudio,
            );
