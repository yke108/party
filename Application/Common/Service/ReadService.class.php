...
class ReadService{
    ...
    public function getPagerList($params){
        ...
        return array(
            'list'=>is_option($list,Status::CollectTypeRead),
            'count'=>$count,
            'page'=>$params['page'],
    ...
    private function setDetail($list)
    {
        if(empty($list)) {
            ...
            $map['cat_type'] = Status::CatTypeRead;
            //搜索分类表 取分类名字
    ...
    public function increaseReadNum($params){
        ...
        if ($params['user_id']) {
            $map = array(
                ...
                'article_type'=>Status::CatTypeRead,
            );
