	...
	public function smsLogByTemplate($id, $mobile, $data = array()){
		...
		if ($template) {
			...
			if ($data['DistributorId']) {
				$distributor_ids = array($data['DistributorId']);
				$data_message = array(
						'type'=>1,
						'title'=>$data['Title'],
						'content'=>$str,
				...
				try{
					$this->distributorMessageService()->addAllMessage($distributor_ids, $data_message);
				}catch(\Exception $e){
					throw new CsException($e->getMessage());
				}
			}
		...
		if ($data['UserId']) {
			...
			if ($user_info['openid']) {
				switch ($data['MsgType']) {
					...
					case Status::MsgTypeDistributorCash: //打款成功
						$cash_apply = $this->cashApplyService()->getInfo($data['ApplyId']);
						$url = DK_DOMAIN.'/wap/index.php?s=/user/recharge/info/log';
						$keywords = array(
								'keyword1'=>$cash_apply['bank_name'],
