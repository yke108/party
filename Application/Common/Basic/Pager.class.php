...
class Pager{
    ...
    private $config  = array(
        'header' => '<div class="dataTables_info" style="display:inline-block; vertical-align:middle;float:left; margin:20px 0;">共 %TOTAL_ROW% 条记录 </div>',
        'prev'   => '上一页',
        'next'   => '下一页',
        'first'  => '首页',
        'last'   => '末页',
        'theme'  => '%HEADER%<ul class="pagination" style="vertical-align:middle;float:right">%FIRST%%UP_PAGE%%LINK_PAGE%%DOWN_PAGE%%END%</ul>',
    );
    ...
    public function show() {
        ...
        $up_page = $up_row > 0 ? '<li class="paginate_button"><a class="prev cs_ajax_link '.$this->is_layout_class.'" cs_id="'.$this->add_id.'" target="_self" href="' . $this->url($up_row) . '">' . $this->config['prev'] . '</a></li>' : '';

        //下一页
        $down_row  = $this->nowPage + 1;
        $down_page = ($down_row <= $this->totalPages) ? '<li class="paginate_button"><a target="_self" cs_id="'.$this->add_id.'" class="next cs_ajax_link '.$this->is_layout_class.'" href="' . $this->url($down_row) . '">' . $this->config['next'] . '</a></li>' : '';

        //第一页
        $the_first = '';
        if($this->totalRows > $this->listRows ){
            $the_first = '<li class="paginate_button"><a class="first cs_ajax_link '.$this->is_layout_class.'" cs_id="'.$this->add_id.'" target="_self" href="' . $this->url(1) . '">' . $this->config['first'] . '</a></li>';
        }

        //最后一页
        $the_end = '';
        if($this->totalRows > $this->listRows ){
            $the_end = '<li class="paginate_button"><a class="end cs_ajax_link '.$this->is_layout_class.'" cs_id="'.$this->add_id.'" href="' . $this->url($this->totalPages) . '">' . $this->config['last'] . '</a></li>';
        }
        ...
        for($i = 1; $i <= $this->rollPage; $i++){
            ...
            if($page > 0 && $page != $this->nowPage){
                ...
                if($page <= $this->totalPages){
                    $link_page .= '<li class="paginate_button"><a class="num cs_ajax_link '.$this->is_layout_class.'" cs_id="'.$this->add_id.'" href="' . $this->url($page) . '">' . $page . '</a></li>';
                }else{
                    break;
                }
            }else{
                if($page > 0 && $this->totalPages != 1){
                    $link_page .= '<li class="paginate_button active"><span class="current">' . $page . '</span></li>';
                }
            }
        }
