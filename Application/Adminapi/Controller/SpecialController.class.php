...
class SpecialController extends FController {
	...
	public function whAction(){
		...
		$result = $this->specialService()->getWhPagerList($params);
		$newlist = [];
		if($result['list'] != []){
			foreach($result['list'] as $k=>$v)
			{
	...
	public function create_or_modifyAction(){
		$post = I('post.');
		try {
			$result = $this->specialService()->createOrModify($post);
			if($post['id']){
				$id = $post['id'];
			}else{
				$id = $result;
			...
			$where = [
				'top_type'=>Status::CollectTypeSpecial,
				'content_id'=>$id,
	...
	public function delAction(){
		...
		try {
			$result = $this->specialService()->delete($params);
			$data = array(
					'Message'=>'删除成功',
			);
		} catch (\Exception $e) {
